...
const S_WIDTH = Math.round(86/0.90);
const S_HEIGHT = Math.round(21/0.90);
const S_SPACE = Math.round(8/0.90);
const PROGRESS_TH = Math.round(25/0.90);
const PROGRESS_R = (DW-PROGRESS_TH)/4-Math.round(6/0.90);
const P_START = 90;
const P_END = 10;
const P_DISABLED = 0.3;
const PROGRESSES = [
    [(DW/4)+Math.round(2/0.90), (DW/4)+Math.round(2/0.90), -P_START, -P_END, 0],
    [((3*DW)/4)+Math.round(2/0.90), DW/4, P_START, P_END, 1],
    [(DW/4)+Math.round(2/0.90), DH-DW/4, -P_START, P_END-180, Math.round(3/0.90)],
    [((3*DW)/4)+Math.round(2/0.90), DH-DW/4, P_START, 180-P_END, Math.round(4/0.90)]
];
...
//NUEVO
const I_SIZE = Math.round(25/0.90);
const IL_SIZE = Math.round(31/0.90);
const I_SPACE_H = Math.round(4/0.90);
const I_SPACE_V = Math.round(14/0.90);
...
const C2_DEFAULT = 10008;//hmUI.data_type.WEATHER;
//NUEVO

const C_POS = [DH-C_SIZE+Math.round(20/0.90), PROGRESS_TH+Math.round(25/0.90)];

const W_SIZE = Math.round(37/0.90);

const S_I_SIZE = Math.round(20/0.90);
const S_I_SPACE = Math.round(12/0.90);
...
WatchFace({
    ...
    initView() {
		...
		if(screenType!=hmSetting.screen_type.AOD)
		{
			...
			let c_opt_types = [
				...opt_types,
				{
					type: 10008,//hmUI.data_type.WEATHER,
			...
			function makeWeather(current_y) {
				...
				let widget = hmUI.createWidget(hmUI.widget.IMG_LEVEL, { // icon
					x: DW/2-Math.round(34/0.90),//(DW-W_SIZE-W_SIZE)/2,
					y: current_y,
					w: Math.round(37/0.90),//-I_SPACE_V,
				...
				widget=hmUI.createWidget(hmUI.widget.TEXT_IMG, { // temperature
					x: DW/2+Math.round(12/0.90),//(DW-W_SIZE+W_SIZE)/2,
					y: current_y+Math.round(8/0.90),//current_y+W_SIZE+I_SPACE_V,
					w: DW/4,
				...
				widget = hmUI.createWidget(hmUI.widget.IMG_CLICK, {
					...
					y: current_y,
					w: W_SIZE,
					h: W_SIZE+I_SPACE_V+Math.round((15-5)/0.90),
					type: hmUI.data_type.WEATHER
				});
			}
			...
			function makeProgress(i, typei) {
				...
				widget = hmUI.createWidget(hmUI.widget.IMG, { // icon
					...
					y: [DW/4+I_SPACE_V+Math.round(5/0.90), DH-DW/4-I_SIZE-I_SPACE_V-Math.round(5/0.90)][Math.floor(i/2) % 2]+7,
				...
				if(EDIT_TYPES[typei]<10006)
				{
					...
					widget=hmUI.createWidget(hmUI.widget.TEXT_IMG, { // text
						...
						y: [DW/4+2*I_SPACE_V+I_SIZE, DH-DW/4-2*I_SPACE_V-I_SIZE-S_HEIGHT][Math.floor(i/2) % 2]+7,
						w: S_WIDTH,
						h: I_SIZE,
						font_array: statNums,
						h_space: 2,
				...
				else if(EDIT_TYPES[typei]==10006) //phoneBattery
				{
					widget=hmUI.createWidget(hmUI.widget.ARC_PROGRESS, { // progress
						...props,
						line_width: PROGRESS_TH,
				...
				else if(EDIT_TYPES[typei]==10007) //TIR
 				{
					widget=hmUI.createWidget(hmUI.widget.ARC_PROGRESS, { // progress
						...props,
						line_width: PROGRESS_TH,
			...
			function makeWidget(cType, current_y) {
				...
				let widget=hmUI.createWidget(hmUI.widget.IMG, { // icon
					x: DW/2-IL_SIZE-Math.round(10/0.90),
					y: current_y-Math.round(5/0.90),
					align_h: hmUI.align.RIGHT,
				...
				if(cType<10006)
				{
					...
					widget=hmUI.createWidget(hmUI.widget.IMG_CLICK, {
						x: DW/2-IL_SIZE-Math.round(2/0.90),
						y: current_y,
						w: 2*IL_SIZE+4,
						h: IL_SIZE,
						type: data_type
				...
				else if(cType==10006) //phoneBattery
				{
					widget=hmUI.createWidget(hmUI.widget.TEXT_IMG, {
						x: DW/2+Math.round(10/0.90),
						y: current_y,
			...
			let dateW=hmUI.createWidget(hmUI.widget.IMG_DATE, {
				day_startX: Math.round((133+80)/0.90),
				day_startY: dateline,
				day_zero: 1,
				day_space: 1,
				...
				month_startX: Math.round((176+80)/0.90),
			...
			for (let i in cTypes) {
				//NUEVO
				if (cTypes[i] === 10008){//hmUI.data_type.WEATHER) {
				//NUEVO
					makeWeather(C_POS[i]);
				} else {
			...
			if (screenType === hmSetting.screen_type.WATCHFACE) 
			{
				let lightWidget=hmUI.createWidget(hmUI.widget.IMG, { 
					x: (DW-Math.round(55/0.90))/2,
					y: Math.round(-10/0.90),
					src: IMG+'bright.png',
					show_level: hmUI.show_level.ONLY_NORMAL
				}).addEventListener(hmUI.event.CLICK_UP, function (info) {
    ...
    updateValuesWidget(watchdripData) {
        ...
        if (bgObj.isHasData()) 
		{
			...
			if(bgObj.tir!=null && bgObj.tir!=undefined)
			{
				i=0;
				while(i<tirWidgets.length)
				{
					tirWidgets[i++].setProperty(hmUI.prop.TEXT, bgObj.tir);
				}
				i=0;
				while(i<tirArcs.length)
				{
					tirArcs[i++].setProperty(hmUI.prop.LEVEL, parseInt(bgObj.tir, 10));
				}			
			}
